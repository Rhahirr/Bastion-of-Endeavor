/obj/proc/analyze_gases(var/atom/A, var/mob/user)
	if(src != A)
		...
		*/
		user.visible_message("<span class='notice'>[ru_cap(user)] [ru_v(user, "использовал")] [ru_case(src, "acase")] на [ru_case(A, "pcase")].</span>")
	...
	if(result && result.len)
		...
		*/
		to_chat(user, "<span class='notice'>Результаты анализа[src == A ? "" : " [ru_case(A, "gcase")]"]:</span>")
		// End of Bastion of Endeavor Translation
	...
	*/
	to_chat(user, "<span class='warning'>На [ru_case(src, "pcase")] моргнула красная лампочка: анализ [ru_case(A, "gcase")] не удался.</span>")
	// End of Bastion of Endeavor Translation
	return 0
...
/proc/atmosanalyzer_scan(var/atom/target, var/datum/gas_mixture/mixture, var/mob/user)
	...
	if (mixture && mixture.total_moles > 0)
		...
		*/
		...
		results += "<span class='notice'>[ru_p(target, "в", 1)] [ru_case(target, "pcase")] пусто!</span>"
		// End of Bastion of Endeavor Translation
