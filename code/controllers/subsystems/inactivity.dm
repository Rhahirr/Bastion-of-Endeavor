...
/datum/controller/subsystem/inactivity/fire(resumed = FALSE)
	...
	while(client_list.len)
		...
		if(C.is_afk(config.kick_inactive MINUTES) && can_kick(C))
			...
			*/
			to_chat_immediate(C, world.time, "<span class='warning'>Вы были неактивны больше [ru_count(config.kick_inactive, "минуты", "минут", "минут")], поэтому были отключены.</span>")
			// End of Bastion of Endeavor Translation
				...
				if(ishuman(C.mob))
					...
					*/
					...
					if(isAI(C.mob))
						...
						*/
						global_announcer.autosay("[ru_cap(A)] [ru_v(A, "был")] перемещён в хранилище ИИ.", "Мониторинг Искуственного Интеллекта")
						// End of Bastion of Endeavor Translation
						A.clear_client()
						/* Bastion of Endeavor Translation
