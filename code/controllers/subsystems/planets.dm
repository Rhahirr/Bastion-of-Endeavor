...
/datum/controller/subsystem/planets/Initialize(timeofday)
	...
	*/
	admin_notice("<span class='danger'>Инициализация погоды на планетах.</span>", R_DEBUG)
	// End of Bastion of Endeavor Translation
	createPlanets()
	..()
	...
	for(var/P in planet_datums)
		...
		for(var/Z in NP.expected_z_levels)
			...
			if(z_to_planet[Z])
				...
				*/
				admin_notice("<span class='danger'>Z[Z] используется более чем одной планетой!</span>", R_DEBUG)
				// End of Bastion of Endeavor Translation
				continue
			z_to_planet[Z] = NP
